...
const defaults = { kicker: document.querySelector('#hero-kicker').textContent, title: 'Ð¦Ó©Ð» Ñ†Ó©Ð»Ð¶Ð´Ó©Ð³ Ò¯Ò¯?', description: document.querySelector('#hero-description').textContent };
...
const stored = JSON.parse(localStorage.getItem('cop17-editor') || '{}');
...
document.querySelector('.save-preview')?.addEventListener('click', () => {
  ...
  localStorage.setItem('cop17-editor', JSON.stringify(content));
...
document.querySelector('.reset-preview')?.addEventListener('click', () => { localStorage.removeItem('cop17-editor'); hero.style.removeProperty('--hero-image'); hero.classList.remove('has-video'); video.removeAttribute('src'); Object.entries(defaults).forEach(([key,value]) => { fields[key].value=value; setText(key,value); }); });
